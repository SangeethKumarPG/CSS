...
If different selectors share a common property and it's value which we use multiple times in a file. CSS variables are a relatively new feature which allows you to put the value into a variable which we can reuse. We should define this variable in the file using:
...
In the position where we want to use the value of the variable we can use like:

```javaScript
selector{
    property: var(--vairble-name);
}
```

Here we used the `:root` pseudo selector because it refers to the entire document. We can also place the variable definition on a specific selector but the variable will be usable only in that selector.   
Inside of the var function where we use the value of the variable we can also provide a fallback value. This value will be used when the value of the variable is not accessible. The syntax is `var(--variable-name, fallbackvalue);` The `--` in-front of the variable name is mandatory.
...
Vendor prefixes are a mechanism which allows the browser vendors to implement an upcoming feature in an early version without breaking the feature when it finally release. Vendor prefixes are special value which only work in some browsers. The vendor prefixes don't change even if the feature is completely rolled out to all browsers. This allows the older browsers to support the features. If a browser doesn't understand a vendor prefix for another browser it will simply ignore it.

For some features that are not even implemented in the browsers the vendor prefixes will not even work. In those cases the `@supports` query can help with that. It requires a property and it's value which will check if that property and value is supported by the browser. If it supports then it will execute the rules defined inside that. 

A polyfill is a javascript package which enables certain CSS features in the browsers which would not be supported otherwise. Polyfills may exist for only some of the CSS features. Polyfills are a javascript package so there is some tradeoffs with this approach. This might lead to degraded performance of the application. 

Different browsers use different defaults. This might cause inconsistencies in styles for different browsers for the same application. You should make sure that the application remains consistent in all the browsers. To fix the in consistency problem by the browser defaults we can use reset libraries like `Normalize.css`. This is a CSS package which we add as the import in the html file. This will override the global browser defaults. This will set features like `box-sizing` to `border-box` etc. Alternatively we can do this manually by setting up a universal selector in your css files. If you use an additional package like normalize.css this might increase the size of your page which will have some performance impact for the website.

There are some important do's and don'ts when choosing class names. 

* For class names use lower case characters separated by - symbols. CSS is case insensitive so classes defined with lower case letters and uppercase letters will have the same meaning.
* Name your classes by feature for example `page-title`. Don't name your class by the style which we are going to apply.

Block Element Modifier is a naming convention for naming CSS classes. If you have a large website with multiple pages chances are high that you might reuse some class names unintentionally. BEM enforces a certain way for naming your classes which will minimize this risk. In this notation we will have the block portion of the element followed by two underscores, followed by the element portion, then two dashes and followed by an optional modifier. The syntax will look like:  
`block__element--modifier` 

example:

`.menu-main__item--size-big`   
In this the block is the main-menu, we are selecting a single item in that menu and we are selecting the item which is big. Even though we have told to avoid naming classes based on the size it is going to apply, we can use it here because the entire text will constitute the class name which will ensure that it is unique.

Sass is superset to CSS. It is a language based on CSS. But is not understood by the browsers. 

SASS and SCSS are essentially the same, their syntax varies slightly. SASS stands for syntactically awesome style sheets. It does not run in the browser. It extends CSS during development only. Managing CSS files and rules for large projects can be cumbersome. It would really help if you have easier condensed syntax. SASS offers these features. We can only use it in development. It needs to be compiled to CSS before production. We use some tools to compile it normal CSS. There are a lot of features offered by SASS, some of them are:  

* Nest CSS rules to shorten our code
* We can use inheritance to inherit from other rules.
* We can use helper functions to add customizations.
* We can use mixins to create reusable snippets of code.
* We can split our code into multiple files easily using partials.
* We can define variables and use them.
* We can use conditions and loops.

To install the compile for SASS we need to use the following command:  
`npm install -g sass` 

To write sass code we need to create a `.sass` file in the project. Sass works without braces and semi-colons. We use indentation to determine which rules apply to which selectors. To make normal CSS code into sass we should remove all the {} and replace them with indentations. The indented rules are applied to the selector which is one level above. If we are using SCSS, it supports normal CSS syntax like braces and semi-colons. Since this method is easier we can use SCSS. For this we should create a filename with `.scss` extension. The normal CSS is also valid SCSS.

One of the most powerful feature of SCSS is nesting of rules. For example we have an element with a class. This element have a child list item tag. Inside the selector of the parent element we can add the li element selector and apply the styles along with the styles which are already applied to the parent element. For example:
...
We can also nest pseudo selectors like :hover, :active etc.

This way we can make the css rules concise by placing all the rules to an element and it's children in a single CSS selector.  
To compile this file we can use the command line the syntax is:

`sass inputfile.scss outputfile.css  
` This will generate the CSS file from the scss file we have defined. The generated CSS file will have the suitable selectors for the nested rules.  
If you don't want to run the above command every time you make a change you can use:  
`sass --watch inputfile.scss:outputfile.css`   
This will start the watch mode which is an ongoing process which we can stop with `ctrl+c`. Now when we change something in the scss file and save that file it will automatically recompile. 

We can also have nested properties in scss. For example we have property names related to one another. Like flex-direction which sets the flex direction and flex-wrap which sets the wrapping of the flex. In normal CSS we can define separate properties for things like these or use short hand. When using scss we have an option to nest properties so it will look like:
...
We can use variables which can be used to store commonly used property values. This is available in CSS also. But when using variables in SCSS it will be compiled to code that does not use CSS variables. To declare variable in SCSS we use `$variable-name: value;`. We can store any type of value in CSS. In the places where we want to use the variables we can use the variable name. For example we can define like:
...
We can also store lists and maps in a variable. A list is a variable that holds more than 1 value. For example in case of the box-shadow we have a list of values. Though we can store each values individually in a variable and construct the value of the property from multiple variables, it can be quite cumbersome. To create a list we just have to initialize a variable with values separated by space.

```javaScript
$border-default: 0.05rem solid $main-color;
```

Even if you have comma separated values it will act as list. 

We also have maps in SCSS. A map is a list where each item have a name by which we can access. A map is constructed by using (). Inside this we create key value pairs. We can provide any key name of your choice. After specifying the key name we provide a colon after which we add the value. We can add more such key value pairs by adding commas between them.  
Example:  
`$colors: (main: #521751, secondary: #fa923f);` 

Sass also supports functions. To get the values from a map we can use the `map-get()` built-in function. This function takes in the name of the map as the first argument and as the second argument we should pass the name of the key whose value we want to access. For example:

```javaScript
$border-default: 0.05rem solid map-get($colors, main);
```

 We can also specify the arguments like:  
` border-bottom: 0.05rem solid map-get($map: $colors, $key: main);`   
Which is also valid.  
Maps make it easier to change the values of properties. We only need to change the value in one place which will be reflected wherever we use the value from the map.

There are a lot of built-in functions in sass. In our page we can have colors that are dependent on other color to have the desired look. For example if we want to use a lighter version of our main color to another element. For this we can use the `lighten()` function. The first argument of this function is a color and the second argument is the amount by which we want to lighten the color. It takes in a value between 0 and 100%.

```javaScript
  background: lighten($color: map-get($colors, main), $amount: 72%);
```

By using like this whenever we change the main color this will also automatically adjust.

Sass also supports simple arithmetics like addition, multiplication, division etc. 

We can also use imports in scss files like in css files. If we import a css file it will leave that import as is. So when we import a css file in the scss file it will also load the imported css file when browser loads the page. We can verify this by checking the network tab. Sometimes we might want to generate a big css file and split it at the time of development which is easier to manage. We can also use variables in the files which we are importing. To enable this we can use partials. To create a partials we should create a file that starts with \_ and it should end with scss. The idea behind a partials is that we can include it in many other files and behind the scenes sass will ensure that it will only get included once and it's values are only used once. We can place our variables in our scss file and place them in the partials file.

To use the values we should import the partials file like:  
`@import "_variables.scss";` 

We can use this multiple scss files. We must make sure that the import is placed in a scss file and not a css file. Because this is not supported by normal css. We don't need to separately compile the file if we are importing the file into the main file which we are watching. 

We can also nest the media queries inside of the selector of an element in scss. When doing so we can avoid the use of selector, we can specify the styles inside of the opening and closing braces of the media query without using the selector. During the compilation of the scss file it will still generate a valid media query with the appropriate selector. Using this feature is optional, the normal way of writing media queries will work even in SCSS.

Inheritance means that if we have a rule set for a given selector, which we also want to use for a different selector we can simply inherit from the selector and apply the style. To extend the values from a selector we use the `@extends` directive inside of the selector where we want to apply the inherited styles. This is a sass only feature.   
The syntax is:

```javaScript
css-selector{
    @extends css-selector__to-extend;
    other rules....;
}
```

After compilation it will generate suitable selectors and the rules automatically.

Mixins are reusable custom functions. We create a mixin using the `@mixin` directive provided by sass. After this we can provide any name as we want. After this we add parenthesis, we can specify arguments inside these if we want. And after this add opening and closing curly brackets. Inside the curly brackets we specify the code which we want to be used when we use the mixins. This can be any scss code or normal css rules without using selectors. After this we can use this mixin where ever we want to enter that code. To use this mixin we should use `@include` directive inside of the css selector where we want to add this code. The syntax will look like:
...
We can also accept arguments to a mixin for this we should specify a variable name which starts with $ sign. We can use this variable inside of the mixin body. The syntax will look like:
...
This can be useful when creating media queries.  
We can also dynamically inject content into the mixin for this we can use `@content` inside of the mixin body. Then in places where we are including the media query, to pass the content we can use the below syntax:
